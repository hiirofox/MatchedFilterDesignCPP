...
namespace AnalyticGradient
{
	...
	private:
		...
		ComplexIIRGradientMagTheta<numOrders> gradCalc;
		...
		struct DefaultMagError : public MagErrorBase
		{
			double Error(int i, double freqhz, double iirmagdb) override
			{
				...
				// 这里延续你原来的风格
				const double ev = e1 * 0.00001 + e2 * 0.99999;
	...
	public:
		MatchedComplexIIRDesignAnalytic()
		{
			coeffs.resize(numParams);
		...
		void SetupAnalogPrototype(AnalogFilterType type, float fc, float q, float gain, float stages) override
		{
			...
			// 重置优化器 basin
			SetupInitialParams();
			optAdam.SetupOptimizer(numParams, coeffsD, 0.0003);
			optLbfgs.SetupOptimizer(numParams, coeffsD, 0.2);
			totalCycles = 0;
			usingLbfgs = false;
		}
		...
		void RunOptimizer(int numCycles, int maxCycles) override
		{
			...
			if (totalCycles > maxCycles * 10.0 / 200.0 && !usingLbfgs)
			{
				usingLbfgs = true;
